...
2. **Add or plan a DB-level invariant so a signal cannot be published unless its company is published.** `docs/DATA_MODEL.md`'s current publication invariants (§"Publication invariants," items 1–7) do not yet state this explicitly, and M6 is the first milestone where it can actually be exercised: a connector-created company stays `draft` while its `new_signal` item can still be `approve`d through the unmodified M4 gate (R1, D-085) — today, in the all-demo dataset, every published signal's company happens to already be published, so this gap has never been triggered.
   ...
   - At minimum, a **public-invisibility test** must prove that connector-imported drafts never appear anywhere on a public route — this is required in M6 regardless of whether the DB-level trigger ships in the first M6A pass.
   - **Update (D-090):** once the trigger exists, "an approved connector-imported `new_signal` whose company remains a draft" is no longer a state that can be constructed at all — `submit_review_action`'s `approve`/`edit_approve` will raise an exception instead, since it sets `publication_status = 'published'` unconditionally today with no company check. D-090 locks the requirement that `submit_review_action` must be reconciled (in a later, separately-planned step, before M6B/M6D expose connector reviewer actions in the UI) so that approving a connector-sourced item records triage without erroring or attempting public visibility. See `docs/DECISIONS.md` D-090 for the full requirement; the exact RPC mechanism is not decided here.
