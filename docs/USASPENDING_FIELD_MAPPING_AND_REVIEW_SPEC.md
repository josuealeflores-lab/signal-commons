...
2. **Person-named-recipient check (Fable R5, before any draft-company creation)** — before branch 3 below runs, test whether `recipient_name` appears to be a natural person (e.g., `"Last, First"` formatting or other individual-name patterns common to USAspending sole-proprietor/individual awardees — exact heuristic finalized in the Entity-Resolution Policy). **If it appears to be a natural person, never auto-create a draft company.** Route to an **`entity_match`** research item with `reason='possible_individual'`, or exclude the record entirely pending human review judgment — a reviewer decides whether and how to proceed. This is a conservative privacy/reputational safeguard, not an entity-matching optimization: an automatically-created "company" profile for what is actually a named individual is a real reputational and privacy risk this project should never take on by default.
3. **No match (and not a person-named recipient) →** create an `is_demo = false` **draft** company: `publication_status='draft'`, `company_type='unclear'` (AI classification is a reviewer judgment), factual placeholder `summary`/`why_it_matters`/`stage`; insert `company_aliases` rows for the UEI and legal name; emit a **`new_company`** research item (queue-only under Option B).
4. **Ambiguous** (partial/conflicting UEI-vs-name) → do **not** guess; emit an **`entity_match`** research item (queue-only). Full rules: Entity-Resolution Policy.
...
- **What "approve" actually means for M6 import signals (Fable R1 fix — see D-085).** The RPC's `published`/`verified` state values are reused unchanged from the M4 gate — that is a **technical side effect** of routing import records through the existing publish gate, not a claim that the record is ready for public display. **In Milestone 6, `approve` on a connector-imported `new_signal` means "triaged and reviewed," not "cleared for public display."** Public readiness for a real company/signal pair depends on a separate publish-support gate that doesn't exist yet (§14). This distinction matters because it is **not self-enforcing today**: nothing currently prevents a signal from carrying `verification_status = 'verified'` from an M6-era review, then quietly becoming publicly visible later purely because its company was independently published by some future mechanism, without a fresh review of the signal itself.
  - **Hard requirement for any future milestone that adds `new_company` publish-gate support:** that milestone **must re-confirm every already-"approved" M6-era import signal** (i.e., re-run it through an explicit review step) **before** the associated real company — and therefore the signal — can become publicly visible. A stale M6 "approval" must never silently go live later without a fresh look. This requirement is binding on that future milestone's plan, not optional cleanup.
...
**Demo/live visual distinction requirement (Fable R2 fix — see D-085).** Once real connector drafts start entering the reviewer queue, that queue will contain a mix of pre-existing `is_demo = true` demo items and new `is_demo = false` real items side by side. **The reviewer queue UI must visually distinguish demo items from real connector items — via a badge, a filter, or both — before any real connector draft is allowed to enter the queue.** A reviewer must never be able to mistake a real, non-demo item for a demo one (or vice versa) at a glance. This is a hard requirement on the M6 reviewer UI, not a nice-to-have; see §13 for the corresponding test requirement.
...
| `verification_status` (import) | `unverified` → `verified` on approve | Human-review axis, separate from evidence strength; see R1's semantics note (§5.4) — "verified" means triaged, not public-ready, for M6 import records |
| Public "why it matters" | reviewer-authored, never invented | Significance is editorial, gated by review |

**Reviewer-UI caveat recommendation (non-blocking, Fable follow-up).** Wherever `evidence_strength = high` is displayed to a reviewer for a connector-imported signal, the UI should carry an inline caveat reiterating that "high" means the *award event* is documented by a Tier-1 source — not that the company's impact or success is established. This is already the documented rationale in this table; the recommendation is to surface it directly in the reviewer UI copy, not only in this document, so reviewers aren't relying on having read this spec.
...
- **Demo/live distinguishability test (Fable R2 fix, required in M6, not deferred):** with a mix of `is_demo = true` and `is_demo = false` items in the queue, assert the reviewer UI renders a visible, distinguishing badge/filter for each — e.g. an accessibility-tree or snapshot assertion that a demo item and a real item are never rendered identically. This must exist **before** any real connector draft is allowed into the queue, per §11.
- **Person-named-recipient test (Fable R5 fix):** a recipient name matching the natural-person heuristic (§5.1) never results in an auto-created draft company; it produces an `entity_match` item with `reason='possible_individual'` (or is excluded), for both contract and assistance award fixtures.
- **Inert-rendering test for untrusted award text (Fable non-blocking follow-up, elevated to required in M6 — not deferred to Stage-2).** Stage-1's reviewer UI already renders real, external award description/title text verbatim (§8). This must be tested **now**, in M6, independent of whether Stage-2 ever lands: assert that award text containing HTML/markdown/script-like content or instruction-like phrasing renders as inert text in the reviewer UI (no injection, no execution, no altered page behavior) and does not influence the Stage-1 filter's own logic path. The previously-planned "Prompt-injection test (when Stage-2 lands)" below is a **distinct, additional** test for the future AI classifier itself, once built — it does not substitute for this one.
...
- **Reviewer throughput / queue-size circuit breaker** — a real consideration raised by Fable's review (a large ingestion run could flood the queue faster than reviewers can triage), but **not a locked requirement yet** — tracked as an open consideration in `docs/DECISIONS.md`'s deferred-decisions list, to be revisited before the first real run if queue volume looks likely to be large.
...
4. **Retention policy** — for stored raw award JSON / excerpts / content hashes (RESEARCH_METHODOLOGY requires a per-source retention policy). **Elevated (Fable follow-up): this must be decided before the first real data pull, not left open indefinitely** — it is a prerequisite for that pull, not a someday-item.
