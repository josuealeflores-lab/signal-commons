...
- **Leaked password protection disabled** (WARN) — a Supabase Auth project setting, not a migration/schema concern. Recommended, zero-code follow-up: enable it in the dashboard (Authentication → Policies) before any real reviewer account is provisioned. Not done as part of this milestone since it's a dashboard toggle outside "apply the same migrations unchanged," not a code change. **Status update (`docs/DECISIONS.md` D-101):** still disabled as of M11 — confirmed unavailable on the project's current Supabase Free plan (Pro Plan or above required). No longer a blocker to M11's own technical completion; remains required before any real production reviewer is activated, resolved via a plan upgrade (preferred) or a separate compensating-controls decision with Cowork/Opus review. This M5-era passage is left otherwise unchanged as a historical record of the original finding.
