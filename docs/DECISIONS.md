...
### D-099 — M10 Public Usability and Trust Foundation Plan: PASS WITH NOTES; plan approved

**Choice:** Cowork/Opus 4.8 High reviewed the M10 plan (per D-098's adopted roadmap: *"M10 — Public Usability & Trust Foundation"*) and returned **PASS WITH NOTES**, with three required plan changes folded into the approved scope below. M10 is a **public usability/trust milestone** — it is explicitly **not** a production AI activation milestone, **not** a real-data publishing milestone, and **not** a schema/RLS/RPC milestone. Its purpose is to make the existing, already-shipped demo more understandable, trustworthy, and discoverable to a public visitor, before any real data, production AI activation, or agent expansion (M11–M15) begins.

**Approved M10 scope:**
1. Add a public `/about` route as a single, combined FAQ/About page.
2. Add a plain-language "what is a signal?" explainer, surfaced on the homepage explainer, the `/signals` list page, and `/about`.
3. Add badge-to-methodology links for the evidence-strength badge, the verification-status badge, and the source-tier label, each linking to its existing definition section on `/methodology`.
4. Make the existing `/methodology` "Corrections" section actionable through a low-risk corrections channel (see the corrections-mailbox gate below) — today it describes the internal audit-trail process only, with no actual submission mechanism.
5. Add `robots.ts`, `sitemap.ts`, and basic OpenGraph metadata (Next.js App Router native conventions, no new dependency) — subject to the demo-indexing/robots-policy requirements below.
6. Add human-written, static (non-DB) sector context for all seven canonical sectors, subject to the equal-treatment guardrails below.
7. Record the search strategy as **Postgres full-text search, deferred** — a decision, not an implementation.
8. Record the signal-slug strategy as a **human-readable, title-derived slug plus a short stable uniqueness suffix**, deferred until immediately before real public publishing (M14) — a decision, not a migration.
9. Add tests for the `/about` page, the corrections link, the three badge/label links, `robots.ts`/`sitemap.ts`, sector-context parity, the no-leakage scan, and smoke coverage — see "Required test coverage" below.

**Approved non-scope (explicit, unchanged from the reviewed plan):** no DB-backed corrections form; no new table; no new RLS policy; no migration of any kind; no search implementation (decision only); no signal-slug migration (decision only); no RSS/feed for now; no production AI activation; no `ANTHROPIC_API_KEY` provisioning; no production reviewer creation or activation; no connector script run or write to `connector-runs/`; no real-data publishing; no agents or agent-capability expansion beyond what M7/M8A already ship; no scheduled or background job of any kind; no public access to private/reviewer-only data; no expansion of service-role runtime access anywhere in the app. Every one of these remains a standing, separately-gated decision this entry does not open.

**Required plan change 1 — demo-indexing / robots policy (Cowork-required):** all current public company/signal content is fictional demo data (unchanged since the project's founding demo-data rule). M10 introduces the site's first SEO/social surfaces (`robots.ts`, `sitemap.ts`, OpenGraph metadata); this must not cause fictional demo content to be presented to crawlers or social cards as if it were real. While the site remains demo-only, the **default policy is `noindex`** — M10 must not optimize fictional demo signals/companies for public search indexing. `robots.ts`, `sitemap.ts`, and OpenGraph metadata must all preserve the demo-only disclosure posture already established by `DemoDataBanner` and the methodology page — none of these new surfaces may imply real, live-monitored content. If OpenGraph metadata is added (per §"Recommended decisions" below, title/description-only), the title/description text must not imply real live AI monitoring or make any real company/signal claim. Individual signal detail URLs (`/signals/[id]`) must **not** be included in `sitemap.ts` while they use raw UUID ids and before the signal-slug strategy above is actually implemented — `sitemap.ts` must prefer **stable structural public routes only** for M10 (e.g. `/`, `/sectors`, `/companies`, `/signals`, `/methodology`, `/about`, and sector/company detail routes, which are already slug-based), unless a future implementation-review step separately approves a more specific safe list. Reviewer/auth routes (`/reviewer/**`, `/auth/**`) must be explicitly disallowed in `robots.ts` and must never appear in `sitemap.ts`. No draft, private, reviewer-only, or connector-private content may appear in `sitemap.ts` under any circumstance — `sitemap.ts` must be built from the same published-only repository read functions the public app already uses, never a raw or service-role table scan.

**Required plan change 2 — corrections-mailbox gate (Cowork-required):** mailto is the approved first corrections-channel mechanism, but it is approved **only for a confirmed, monitored, non-personal corrections alias** — never a personal email address, and never a placeholder or unmonitored address. The preferred address format is a domain-based alias such as `corrections@signalcommons.org`. The exact mailbox/domain remains a **product/ops decision** that must be confirmed before the live mailto link is implemented — this entry does not itself confirm that address. Consequently: the rest of M10 may be planned and implemented in full, but **M10 cannot meet its exit gate until the corrections channel is actually live** with a confirmed, monitored, non-personal alias. If implementation reaches this point before the address is confirmed, every other M10 item may still be merged, but the corrections-channel item — and therefore the milestone's exit gate — remains open until the address is confirmed.

**Required plan change 3 — sector-context equal-treatment guardrails (Cowork-required):** sector context must be written for **all seven canonical sectors** (`politics-civic-technology`, `government-operations`, `agriculture`, `healthcare`, `education`, `nonprofits`, `climate-energy`, per `CLAUDE.md`), with **comparable structure, depth, length, and tone** across all seven — no sector may receive materially more or less space, more concrete examples, or a more confident tone than another. Copy must be **neutral and non-ranking** — it must not imply that one sector is more important, more advanced, or more credible than another. Copy must be **demo-honest**: it must not describe fictional sector achievements, companies, or outcomes as real, consistent with the project's standing demo-data rule. Sectors should appear in canonical order (per `CLAUDE.md`'s numbered list) or another neutral, non-ranking order, with equal visual weight in the UI — no sector card, section, or list position may be styled or sized to appear more prominent than another. The required test suite must include a **parity check** asserting all seven canonical slugs have sector-context copy present and that each entry's length falls within a comparable range (e.g. no entry more than roughly 1.5x the shortest entry's length) — this is a structural, automatable proxy for the equal-treatment requirement, not a substitute for human review of tone.

**Recommended decisions (adopted):**
- Use `/about` as the single, combined FAQ/About route — not a separate `/faq` route, for now, since the content overlaps heavily with `/methodology` and splitting it would fragment a small amount of content across three pages.
- Use **title/description-only** OpenGraph metadata for M10, because no approved brand/OG image asset exists today (confirmed — only `src/app/favicon.ico` exists); an image may be added later, once one is approved, without requiring another architecture change.
- Search is **strategy-only** in M10; the intended future mechanism is **Postgres full-text search** (`tsvector` + GIN index on `companies`/`signals`), to be implemented in a later milestone once real data volume justifies it.
- Signal slugs are **strategy-only** in M10; the future format is a **title-derived, human-readable slug plus a short stable uniqueness suffix** (mirroring the existing `unique not null` `slug` column pattern already used by `sectors` and `companies`), to be generated and stored **before** any record is ever really publicly published (M14) and **not casually regenerated after publication** — a published signal's slug must remain stable once real, to avoid breaking external links/citations to real public content.
- RSS/feed is deferred — no clear near-term consumer while all public content remains demo/fictional, and it would add a new route and XML-generation surface for no proven current value.

**Required test coverage for M10 implementation:** *Hermetic* — `/about` renders all required sections (per D-099's approved scope item 1); "what is a signal?" copy appears on the homepage, `/signals`, and `/about`, and links correctly between them; the `/methodology` Corrections section contains an actionable corrections link **only once a confirmed alias is available** — a test must assert the shipped mailto address is never a placeholder, a personal-looking address, or an unconfirmed value (e.g. reject patterns like `example.com`, `TODO`, `noreply@`, or any address not matching the confirmed domain-alias decision above); the evidence-strength badge, verification-status badge, and source-tier label each link to their correct `/methodology#...` definition anchor; `robots.ts` policy matches the demo-only `noindex`/disallow posture described above, including explicit disallow of `/reviewer` and `/auth`; `sitemap.ts` includes only the approved stable public routes and excludes reviewer/auth/private/draft routes and raw signal-UUID detail URLs, unless a later implementation-review step separately approves a more specific safe list; OpenGraph metadata includes title/description only and preserves demo-only honesty (no real-content or real-monitoring implication); a sector-context **parity test** covers all seven canonical slugs with comparable length, per Required plan change 3; `tests/lib/no-connector-leakage.test.ts`'s `SCANNED_DIRS` is extended to cover any new `src/lib/content` directory introduced by this milestone. *e2e (Playwright)* — `/about` is reachable from public nav/footer and renders; no reviewer/auth link ever appears in public nav or footer (extending `e2e/smoke.spec.ts`'s existing coverage). *`test:db`* — **not required**: M10 introduces no DB/RLS/schema/RPC change of any kind; the search and signal-slug items are decisions only.

**M10 exit gate (restated from D-098, made explicit here):** quality gate (`lint`/`typecheck`/`test`/`build`) green; an accessibility check performed; the corrections channel live with a confirmed, monitored, non-personal alias (per Required plan change 2 — this is the one item that can hold the exit gate open even after everything else merges); a focused Cowork/Opus 4.8 High implementation review complete; no production AI activation; no real-data publishing; no schema/RLS/RPC changes anywhere in this milestone.

**Implementation sequence:**
1. This D-099 entry drafted.
2. Docs-only quality gates run (`lint`/`typecheck`/`test`/`build`).
3. D-099 committed and merged as a docs-only checkpoint, if separately approved.
4. Confirm the corrections mailbox/domain (product/ops decision, per Required plan change 2) — a prerequisite for implementing the live mailto link, though other M10 items do not need to wait on it to begin implementation.
5. M10 implementation (the nine approved scope items above).
6. Local gates (`lint`/`typecheck`/`test`/`build`); no `test:db` step (see "Required test coverage").
7. An accessibility check performed.
8. A focused Cowork/Opus 4.8 High implementation review, covering: `/about` and "what is a signal?" copy honesty; the corrections-channel mechanism and confirmed address; badge-link wording/`aria-label`s; sector-context equal-treatment (Required plan change 3); `robots.ts`/`sitemap.ts` scope (Required plan change 1); OpenGraph copy honesty.
9. Commit/PR/merge to `main` via feature branch (same pattern as M6–M9, D-098).
10. Post-merge cleanup (same local git sequence as every prior milestone).
11. A read-only M10 demo-readiness check as the last step, mirroring the M6/M7/M9 production-readiness-check pattern — confirming the new public routes render correctly, the corrections link works, `robots.ts`/`sitemap.ts` are correct, and no production/reviewer/AI state changed.
...
**Addendum (post-implementation, Cowork/Opus PASS on Phase B implementation): idempotency key generation is server-side, not client-side.** The implementation generates `p_idempotency_key` server-side with `crypto.randomUUID()`, once per Server Action / RPC-recording invocation (`src/lib/review/actions.ts`'s `callSubmitReviewAction`, `src/lib/copilot/run-analysis.ts`'s `runAnalysisAndRecord`). The app does not accept a client-provided idempotency key from the UI. This is a deliberate refinement of this entry's earlier "client-generated key" wording (§5 of the M11 plan) for the project's actual Server Action architecture — a plain form POST has no client-side retry logic that would need a key stable across separate submission attempts, unlike the REST/XHR-retry client model that wording originally assumed.

This refinement still protects the two guarantees Phase B needs today: within-invocation RPC retry/idempotency (a single Server Action's own transient network retry against Supabase reuses the same key, since the key is generated once per invocation and closed over for the duration of that call), and it installs the full DB-side idempotency machinery (`idempotency_keys` table, `INSERT ... ON CONFLICT`-based concurrency control, replay/conflict detection) exactly as designed.

What it does **not** protect: a reviewer's own cross-invocation double-submit (e.g., clicking "Approve" twice in quick succession before the first request resolves generates two distinct server-side keys, not one). This gap is currently mitigated at the UI layer instead — disabled-while-pending buttons on submit, and, for `submit_review_action` specifically, the research-item status state machine itself (a second approve/reject attempt against an already-transitioned item is rejected by the existing status-guard logic, independent of idempotency keys). For `record_copilot_analysis`, a rare double-submit produces at most a low-risk duplicate advisory `copilot_analyses` row (never a duplicate publish action), and is likewise button-mitigated.
